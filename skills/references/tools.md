...
| `engine` | `'claude'` \| `'codex'` \| `'codex-app'` \| `'gemini'` \| `'kimi'` \| `'cursor'` \| `'opencode'` \| `'custom'` | Engine to use (default: `claude`). `kimi` wraps Moonshot Kimi Code. `opencode` wraps sst/opencode (pass model as `provider/model`). Use `custom` with `customEngine` for any CLI. |
